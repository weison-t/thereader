...
export async function POST(req: NextRequest) {
  try {
    ...
    if (action === "process") {
      ...
      // Pull rows from response_result so we capture raw scoring output
      ...
      for (const r of rows) {
        ...
        const payload = {
          ...
          proper_empathy_acknowledgement: toNumberFromScoreCell(r.proper_empathy___acknowledgement ?? r.proper_empathy_acknowledgement),
          breach_confidentiality: String((r.breach_of_confidentiality___auto_failed ?? r.breach_confidentiality_auto_failed) || '').toLowerCase() === 'true',
          rudeness_unprofessionalism: String((r.rudeness_or_unprofessionalism___auto_failed ?? r.rudeness_unprofessionalism_auto_failed) || '').toLowerCase() === 'true',
          overall_chat_handling_customer_experience: toNumberFromScoreCell(r.overall_chat_handling_customer_experience),
          scoring: typeof r.final_score === 'number' ? r.final_score : Number(r.final_score ?? null),
